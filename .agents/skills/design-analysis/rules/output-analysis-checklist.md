...
#### 文字
- 标题文字：内容「首页」，字体 PingFang SC，字号 16px，字重 500，颜色 #13131c，行高 24px（精确）

#### 图片
- Logo：尺寸 120×40，位置 (20, 20)，比例 3:1（精确）

#### 布局
- 位置：(0, 0)
- 尺寸：1440×80
- 间距：padding 20px（估算），gap 24px（估算）
...
### 关系型区域补充：字段/属性映射、表格、左右列

当区域存在字段到属性、左列到右列、表格行到详情行等对应关系时，必须在该区域下输出行级表：

| 行 | 源字段/左侧项 | 源类型/标签 | 目标字段/右侧项 | 目标控件/状态 | 字数/校验 | 操作按钮 | 证据等级 |
|----|---------------|-------------|-----------------|---------------|-----------|----------|----------|
| 1 |               |             |                 |               |           |          |          |

同时记录：
- 顶部控制：主键、标题键、搜索/筛选/批量按钮的默认值、占位符、可选项。
- 行级关系：是否一一对齐、是否允许删除/新增、是否存在主键/必填/禁用状态。
- 若仅凭截图无法确认，标注“待确认”，不得用猜测值填充。
...
|------|------|--------|----------|----------------|
| 导航栏 | 字体家族 | 待确认 | 待确认 | 与设计确认是否为 PingFang SC |
| 主内容区 | 卡片间距 | 24px | 估算 | 通过多状态截图复核 |

> 证据等级说明：精确（可直接读取）/ 估算（可推导）/ 待确认（证据不足，禁止伪精确）
```
