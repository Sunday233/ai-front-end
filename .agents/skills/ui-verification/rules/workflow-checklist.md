...
### 验收中（必须落地）
- [ ] Codex 中已按 `browser:control-in-app-browser` 连接 in-app Browser (`iab`) 并打开目标页面；Cursor 中已用 **Browser** 打开目标页面
- [ ] 已获取 Browser 截图或快照；若降级，已记录失败阶段（skill/脚本/运行时/`iab`/目标 URL 导航）和替代工具证据（详见 `tools-browser-navigation.md`）
...
- [ ] 表格、列表、左右列映射、字段-属性映射已逐行核对名称、顺序、状态、操作按钮和对齐
- [ ] 差异已按 P0（布局/层级/内容）/P1/P2 分类并写入「UI 问题清单」
- [ ] **已检查布局常识问题**：页面容器宽度、横向列表铺满、元素完整性、对齐方式等（详见 `writing-*.md` 和 `errors-*.md`）

### 修复后
- [ ] 已再次用 Codex in-app Browser / **Browser** 打开页面并重新与设计稿比对；Browser 目标 URL 仍失败时沿用已记录的降级工具（详见 `tools-browser-navigation.md`）
- [ ] 问题清单状态与验收结论已更新
