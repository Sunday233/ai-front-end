...
description: 指导在前端项目中按团队规范创建和拆分 Vue 3 组件，包括目录结构、文件命名、样式与主题变量使用。当前端需要新增或重构组件时使用本技能。
---
...
<template>
  <button
    type="button"
    :class="[styles.shentuButton, props.className]"
    @click="props.onClick"
  >
    <slot />
  </button>
...
const props = defineProps<ShentuButtonProps>();
</script>
```
...
export {default as ShentuButton} from './shentu-button/index.vue';
```
...
- [ ] 是否使用了 `index.vue + index.module.scss` 的结构？
