...
description: 指导在前端项目中正确使用 Ant Design Vue 与自定义主题 CSS 变量，避免硬编码颜色并保证暗色/浅色主题切换一致性。当前端编写或修改样式时使用本技能。
---
...
- UI 还原时，表单控件、标签、禁用态、只读态、错误态、删除/新增按钮等状态样式必须按设计稿或基础设计规范核对；不要只套用默认组件样式后跳过验收。

---

## 常用 Ant Design Vue 主题变量
...
- [ ] 是否避免在组件/页面样式中写死颜色值？
- [ ] 表格/列表/左右映射/字段属性映射中的标签、控件状态、按钮尺寸和对齐是否与设计稿一致？
