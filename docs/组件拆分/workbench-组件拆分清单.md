...
| `AppShell` | 公共布局 | `src/layout/app-shell/index.vue` | 左侧品牌、菜单、资源数量、底部保存区与主内容容器 | 三个页面真实复用，允许抽公共布局 | 左侧固定导航 |
| `WorkbenchHeader` | 页面级 | `src/views/workbench/components/workbench-header/index.vue` | 面包屑、搜索框、新建按钮、下拉菜单 | 页面专用 | 顶部操作区 |
| `CreateResourceMenu` | 页面级候选复用 | `src/views/workbench/components/create-resource-menu/index.vue` | 新建对象类型/链接/动作/对象类型组选项 | 列表页若复用再抽公共 | 新建下拉菜单 |
...
|-----------|--------------|----------|------------|-----------|------------|
| `AppShell` | `resourceMenus[]` | `resourceType`,`resourceCount`,`permissionCode` | 菜单文案、数量标签、路由 | active/hover/hidden | 菜单顺序、数量、选中态一致 |
...
- `AppShell` 因当前三页真实复用，可放在 `src/layout/app-shell/`。
