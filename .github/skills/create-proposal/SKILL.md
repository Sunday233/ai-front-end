...
若交付形态是**页面**或**功能组件**，根据 `.github/instructions/04-组件规范.instructions.md` 判断组件放置位置：
...
- 图标/图片未定时用占位元素（见 `.github/instructions/08-通用约束.instructions.md`）
...
- **已提供接口**：按 `.github/instructions/03-项目结构.instructions.md` 等规范，在 tasks 中安排 `interfaces/`、`http/` 等。
...
- **使用技能**：`.github/skills/ui-verification/SKILL.md`
...
- `.github/instructions/03-项目结构.instructions.md` - 目录结构、Mock 数据策略
- `.github/instructions/04-组件规范.instructions.md` - 组件放置决策
- `.github/instructions/08-通用约束.instructions.md` - 占位元素等
- `.github/instructions/09-样式规范.instructions.md` - 设计稿颜色提取
- `.github/skills/design-analysis/SKILL.md` - 设计稿分析（有设计稿时使用，产出 UI 分析清单）
- `.github/skills/ui-verification/SKILL.md` - UI 验收（实现后需验收时使用）
