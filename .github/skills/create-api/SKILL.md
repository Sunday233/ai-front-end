...
- 返回值一般是
  \{
    ...
    data: {...}
  \}
  若项目对响应进行了统一解包（例如拦截器只返回 `data`），此处只需定义业务 `data` 结构。
...
export const getBannerDetail = async (id: number) => {
  return httpClient.get<GetBannerDetailResponse>(`/backend/banner/${id}`);
};

export const createBanner = async (data: CreateBannerParams) => {
  return httpClient.post<unknown>('/backend/banner', data);
};

export const updateBanner = async (data: UpdateBannerParams) => {
  return httpClient.put<unknown>(`/backend/banner/${data.id}`, data);
};

export const deleteBanner = async (id: number) => {
  return httpClient.delete<unknown>(`/backend/banner/${id}`);
};
```
...
- 接口错误由 `httpClient`（axios 实例）拦截器统一处理，业务代码中**禁止重复**加 `message.error` 等错误提示。
