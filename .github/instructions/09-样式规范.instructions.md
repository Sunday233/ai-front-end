---
alwaysApply: false
description: 项目的样式规范，包括 SCSS Modules 使用、Ant Design 与自定义主题 CSS 变量。当编写组件样式或主题适配时读取此规则。
---

# 样式规范

## 基础原则

- 组件库使用 Ant Design Vue
- 页面/组件样式默认采用 `<style scoped lang="scss">`
- 仅在样式需要跨组件复用时使用 CSS Modules（如 `index.module.scss`）
- 使用 `class` 或 `:class` 绑定负责动态 class 管理
- UI 交互保持 Ant Design Vue + 自研组件体验一致

## 主题色 CSS 变量（NON-NEGOTIABLE）

在未使用 Ant Design Vue 组件的场景下，自定义样式**必须**使用主题色 CSS 变量，**严格禁止**硬编码颜色值：

- **Ant Design Vue 变量**：`var(--ant-color-primary)`、`var(--ant-color-text)`、`var(--ant-color-bg-container)`、`var(--ant-color-border)`
- **自定义变量**：`var(--shentu-card-bg)`、`var(--shentu-border-radius)`、`var(--shentu-cn-family)`

确保自定义组件能够正确响应主题切换（浅色/暗色模式）。

如需了解主题变量的具体使用方式，请使用技能文件：

- `.agents/skills/theme-variables/SKILL.md`

## 设计稿颜色提取规范

### 常见颜色属性

| 属性 | 含义 | 示例 |
|------|------|------|
| `fill` | 背景色 | `"#13131c73"` |
| `stroke` | 边框色 | `"#5f50ffff"` |
| `opacity` | 透明度 | `0.5` |

### 特殊效果

| 属性 | 含义 | 提取方式 |
|------|------|----------|
| `background_blur` | 毛玻璃效果 | CSS `backdrop-filter: blur()` |
| `shadow` | 阴影效果 | CSS `box-shadow` |
| `cornerRadius` | 圆角 | CSS `border-radius` |

### 提取步骤

1. 定位目标元素（按钮、卡片等）
2. 查找 `fill` 属性获取背景色
3. 查找 `effect` 字段（模糊、阴影等）
4. 记录文字颜色（通常在 `children` 的 `text` 元素中）

### 示例

设计稿按钮：
```
"fill": "#13131c73",
"effect": {"type": "background_blur", "radius": 17.5}
```

对应 CSS：
```scss
.button {
  background-color: rgba(19, 19, 28, 0.45);
  backdrop-filter: blur(17.5px);
}
```

**注意**：带有透明度的颜色需要转换为 RGBA 格式。例如 `#13131c73` 中 `73` 是透明度（十六进制），转换为十进制约 0.45。
